    ...
    <>
      {mobileOpen ? (
        <>
          ...
          <div className="fixed left-1/2 top-2 z-50 w-[min(97%,1408px)] -translate-x-1/2 rounded-lg bg-white p-4 shadow-lg sm:hidden">
            ...
            <div className="mt-3 flex flex-col gap-2">
              {navItems.map(({ label, icon, isActive }) => (
                <CustomButton
                  ...
                  onClick={() => setMobileOpen(false)}
                >
                  <span className="mr-2 text-[16px]">{icon}</span>
                  {label}
                </CustomButton>
              ))}
            </div>
